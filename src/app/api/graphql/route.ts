...
export async function POST(req: Request) {
  try {
    ...
    if (query.includes('Register') || query.includes('CreateUser')) {
      ...
      if (!payload?.email) {
        return NextResponse.json({ errors: [{ message: 'Missing input.email' }] }, { status: 400 })
      }
    ...
    if (query.includes('UpdateTask')) {
      ...
      const taskStatus: TaskStatus = (status as TaskStatus) ?? TaskStatus.Backlog
    ...
    return NextResponse.json({ errors: [{ message: 'Unknown query operation' }] }, { status: 400 })
  } catch (err) {
